odoo.define('de_pos_measures_guide.notes_pos', function (require) {
    "use strict";
    ...
    var NotePopupWidget = PopupWidget.extend({
        ...
        renderElement: function () {
            ...
            for (var note in this.pos.order_note_by_id) {
                $('#note_select').append(this.pos.order_note_by_id[note].pos_note + "= \n")
                  //  .attr("id", this.pos.order_note_by_id[note].id)
                  //  .attr("class", "note_option"))
                
                //$('#note_select').append($("<input type=text style=width:461;>" + "</input><br/>").attr("value", this.pos.order_note_by_id[note].pos_note)
                  //  .attr("id", this.pos.order_note_by_id[note].id)
                  //  .attr("class", "note_option"))
            }
        },
        ...
        click_confirm: function (event) {
            ...
            line.set_note($('#note_select').val());
            this.gui.close_popup();
        },
        click_option: function (event) {
            event.preventDefault();
            event.stopPropagation();
            var old_text = $('#note_select').val();
