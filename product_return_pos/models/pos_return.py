...
class PosOrderReturn(models.Model):
    ...
    def get_lines(self, ref):
        result = []
        # ref = self.search([('return_ref', '=', ref)], limit=1)
        print('ref', ref)
    ...
    def _order_fields(self, ui_order):
        ...
        pos_order = self.env['pos.order'].search([('pos_reference', '=', ui_order.get('return_ref'))])
        for value in ui_order:
            values.append(value)
        self.return_data_fbi(ui_order)
        print('list', values)
        # self.return_data_from_fbr(ui_order)
