...
class PosPayment(models.Model):
	...
	session_id = fields.Many2one('pos.session', string='Session', readonly=False)
	session_ids = fields.Many2one('pos.session', string='Session', readonly=False)
...
class POSOrderInherit(models.Model):
	...
	session_id = fields.Many2one(
		...
		domain="[]", states={'draft': [('readonly', False)]},
		readonly=True)
	secondary_session = fields.Many2one('pos.session')
		...
		for order in orders:
			...
			for pos_order in pos_order_ids:
				if pos_order.is_reserved:
					session_ids[pos_order.id] = pos_order.session_id.id
					for line in pos_order.lines:
						# if 'pos_session_id' in order:
	...
	def _payment_fields(self, order, ui_paymentline):
		...
		return {
			...
			'session_ids' : order.session_id.id,
		}
	...
	def pay_reserved_amount(self,select_journal_id,pay_amount,cash_jrnl_id,session_id):
		...
		for res in payment:
			if res.session_ids:
				res.session_id = res.session_ids
		args = {
