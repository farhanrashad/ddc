...
exports.Orderline = Backbone.Model.extend({
    ...
    get_unit_price: function(){
        ...
        var unit_price = round_pr(this.price, 1);
//        console.log("---1--",unit_price,this.price)
//        if (this.discount){
//            var discount_price = (((unit_price * this.quantity) * this.discount) / 100)
//            console.log("---123--",unit_price,this.discountStr,this.discount);
...
//        }
    ...
    get_display_price: function(){
        if (this.pos.config.iface_tax_included === 'total') {
        	var price_with_tax = round_pr(this.get_price_with_tax(), 1);
            return price_with_tax.toFixed(0);
        } else {
        	var base_price = round_pr(this.get_base_price(), 1);
            return base_price.toFixed(0);
        }
    },
     ...
     */
    _compute_all: function(tax, base_amount, quantity, price_include) {
        if(price_include === undefined)
            price_include = tax.price_include;
        if (tax.amount_type === 'fixed') {
            var sign_base_amount = base_amount >= 0 ? 1 : -1;
            return (Math.abs(tax.amount) * sign_base_amount) * quantity;
        }
        if ((tax.amount_type === 'percent' && !price_include) || (tax.amount_type === 'division' && tax.price_include)){
