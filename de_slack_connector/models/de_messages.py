            ...
            if is_slack == 'True':
                ...
                if values['body'] == 'Contact created':
                    ...
                    if values['model'] == 'mail.channel':
                        if self.env.user.de_sk_user_id and self.env.user.de_sk_user_name:
                            ...
                            if channel.channel_type == 'channel' and values['message_type'] != 'notification':
                                ...
                                slack_user = self.env['mail.channel'].search([('id', '=', values['res_id'])]).channel_partner_ids[0]
                            ...
                            raise UserError(
                                _('Please invite logged in user to slack before sending a message or please '
                                  'disable slack to just send a normal message!'))
                            # raise ValueError('Please invite logged in user to slack before sending a message or please '
                            #                  'disable slack to just send a normal message.')
                        return super(MailMessage, self).create(values)
                    else:
                        return super(MailMessage, self).create(values)
